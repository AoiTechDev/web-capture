          ...
          {useCases.map((useCase, index) => (
            ...
            >
              <div className="flex items-center mb-6">
                <div
                  className={`w-12 h-12 ${useCase.gradient} rounded-xl flex items-center justify-center mr-4`}
                >
                  {useCase.icon}
                </div>
              ...
              <p className="text-[var(--text-muted)] mb-4">{useCase.description}</p>
              {useCase.codeExample && (
                <div className="font-mono text-sm text-[var(--blue-400)] bg-[var(--bg)] p-3 rounded-lg">
                  {useCase.codeExample}
                </div>
              )}
              {useCase.colors && (
                <div className="flex space-x-2">
                  {useCase.colors.map((color, colorIndex) => (
                    <div
                      key={colorIndex}
                      className={`w-8 h-8 ${color} rounded`}
                    />
                  ))}
                </div>
              )}
